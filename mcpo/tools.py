...
class GetTagsToolHandler(ToolHandler):
    ...
    def get_tool_description(self):
        return Tool(
            name=self.name,
            description="Get all tags used across the Obsidian vault, including inline (#tag) and frontmatter tags with counts.",
            inputSchema={
                "type": "object",
                "properties": {},
                "required": []
...
class PutContentToolHandler(ToolHandler):
    ...
    def get_tool_description(self):
        return Tool(
            name=self.name,
            description="Create a new file or completely overwrite an existing file in the vault.",
            inputSchema={
                "type": "object",
                "properties": {
                    "filepath": {
    ...
    def run_tool(self, args: dict) -> Sequence[TextContent | ImageContent | EmbeddedResource]:
        ...
        api = obsidian.Obsidian(api_key=api_key)
        url = f"{api.get_base_url()}/vault/{args['filepath']}"
        import requests
        response = requests.put(
            url,
            headers=api._get_headers() | {'Content-Type': 'text/markdown'},
            data=args["content"],
        ...
        return [
            TextContent(
                type="text",
                text=f"Successfully wrote content to {args['filepath']}"
            )
        ]
